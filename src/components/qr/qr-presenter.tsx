...
export function QrPresenter() {
  ...
  // doRender syncs the canvas (external system) and the animation layers
  // from the active QR. It's a stable useCallback, so the effect just
  // re-invokes it whenever its inputs change.
  useEffect(() => {
    doRender();
  }, [doRender]);
  ...
  return (
    <main className={`${styles.shell} ${lightCls}`} onClick={() => setOverlayVisible((v) => !v)}>
      ...
      <div className={styles.qrStage}>
        {isAnimated && (
          <QrAnimatedPreview layers={layers} style={activeStyle} size={activeStyle.qrSize} />
        )}
        <canvas
          ref={canvasRef}
